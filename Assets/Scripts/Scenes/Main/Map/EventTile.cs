...
public class EventTile : POITile
{
    ...
    void AncientTreasure()
    {
        ...
        MessageLogUI.Instance?.AddMessage("触发事件【古代宝藏】: 获得 200 金币！");
    }
    ...
    void WanderingMerchant()
    {
        ...
        MessageLogUI.Instance?.AddMessage("触发事件【流浪商人】: 获得 150 建材！");
    }
    ...
    void HeroBlessing()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            Card card = CreateCard(p.race, 0);
            p.deck.AddCard(card);
            Debug.Log($"触发事件【英雄遇仙】: 获得 {card.cardName} Lv1！");
            MessageLogUI.Instance?.AddMessage($"触发事件【英雄遇仙】: 获得 {card.cardName} Lv1！");
        }
    }
    ...
    void BanditRaid()
    {
        ...
        Debug.Log($"遭遇强盗！损失 {lose} 金币");
        MessageLogUI.Instance?.AddMessage($"遭遇强盗！损失 {lose} 金币");
    }
    ...
    void Sandstorm()
    {
        ...
        MessageLogUI.Instance?.AddMessage("触发事件【沙尘暴】: 本回合无法再进行主操作！");
    }
    ...
    void Plague()
    {
        ...
        MessageLogUI.Instance?.AddMessage(removed > 0
            ? $"触发事件【瘟疫】: 损失 {removed} 张卡牌（每种至少保留1张）！"
            : "触发事件【瘟疫】: 每种兵种仅剩1张，无法再减少！");
    }
